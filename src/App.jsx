...
export default function App() {
  ...
  const filtered = drawings.filter(d => {
    ...
    if (filterYear) {
      if (!d.scheduled_date || !d.scheduled_date.startsWith(filterYear)) return false;
    }
    return true;
  }).sort((a, b) => {
          ...
          {editMode ? (
            <>
              ...
              <button onClick={() => confirmDelete(selected)}
                style={{ width:"100%",background:"transparent",color:"#ef4444",border:"1px solid #7f1d1d",borderRadius:8,padding:"11px 0",fontWeight:600,fontSize:13,cursor:"pointer" }}>
                🗑 この案件を削除
              </button>
            </>
          ) : (
            <>
                  ...
                  return (
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderBottom:i<files.length-1?"1px solid #1e293b":"none" }}>
                      ...
                      <div style={{ display:"flex",gap:5,flexShrink:0 }}>
                        ...
                        <button onClick={() => handleDownload(f.path, f.name)} style={{ background:"rgba(29,78,216,0.12)",color:"#60a5fa",border:"none",borderRadius:6,padding:"5px 10px",fontSize:11,cursor:"pointer" }}>⬇</button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ background:"#0f172a",borderRadius:10,padding:14,marginBottom:14,border:"1px solid #1e3a5f" }}>
                ...
                <div style={{ display:"flex",gap:6,marginBottom:10,flexWrap:"wrap" }}>
                  {Object.entries(PROD_STATUS).map(([key, info]) => (
                    <button key={key}
                      onClick={() => updateProd(d.id, key, key === "done" ? (d.prod_date || new Date().toISOString().split("T")[0]) : "")}
                ...
                {d.prod_status === "done" && (
                  <>
                    ...
                    <input type="date" value={d.prod_date || ""} onChange={e => updateProd(d.id, "done", e.target.value)}
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#0f172a",color:"#e2e8f0",fontFamily:"'Noto Sans JP','Hiragino Kaku Gothic ProN',sans-serif" }}>
        ...
        <main style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
          <div style={{ background:"#1e293b",borderBottom:"1px solid #334155",padding:"10px 12px",display:"flex",alignItems:"center",gap:8 }}>
            <div style={{ flex:1,position:"relative" }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="案件名・番号・タグで検索..."
                style={{ ...inp, padding:"8px 10px 8px 30px" }}/>
            </div>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)}
              style={{ background:"#0f172a",border:"1px solid #334155",color:"#94a3b8",borderRadius:8,padding:"8px",fontSize:12,cursor:"pointer",flexShrink:0 }}>
            ...
            <div style={{ display:"flex",border:"1px solid #334155",borderRadius:8,overflow:"hidden",flexShrink:0 }}>
              {["grid", "list"].map(v => (
                <button key={v} onClick={() => setView(v)}
                  style={{ padding:"7px 10px",background:view===v?"#334155":"transparent",border:"none",color:view===v?"#e2e8f0":"#64748b",cursor:"pointer",fontSize:14 }}>
                  {v === "grid" ? "⊞" : "≡"}
                </button>
              ))}
            </div>
                      ...
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:5 }}>
                        <div>
                          ...
                          <div style={{ fontSize:11,color:"#475569",marginTop:1 }}>
                            {d.number} · 🏢 {d.client}{d.scheduled_date ? ` · 📅 ${d.scheduled_date}` : ""}
                          </div>
                        </div>
      ...
      {pwModal && (
        <PasswordModal
          title={pwModal.action==="add"?"客先を追加":pwModal.action==="delete"?"客先を削除":pwModal.action==="reorder"?"並び順を保存":"変更を保存"}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}
          ...
          <div style={{ background:"#1e293b",border:"1px solid #334155",borderRadius:16,padding:28,width:"100%",maxWidth:380 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize:12,color:"#94a3b8",textAlign:"center",marginBottom:20 }}>
              を削除してもよいですか？<br/>添付ファイルも含めて削除されます。
            </div>
      ...
      {showClientMgr && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}
          onClick={() => setShowClientMgr(false)}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:8,marginBottom:16 }}>
              {clients.map((c, i) => (
                <div key={i}
                  ...
                  <input value={clientEdit[i] !== undefined ? clientEdit[i] : c}
                    onChange={e => setClientEdit(prev => ({ ...prev, [i]: e.target.value }))}
                    style={{ ...inp, flex:1, padding:"8px 10px" }}/>
                  <button onClick={() => requestClientAction("delete", i)}
                    style={{ background:"#7f1d1d",border:"none",borderRadius:8,color:"#fca5a5",width:34,height:34,cursor:"pointer",fontSize:15,flexShrink:0 }}>🗑</button>
                </div>
              ))}
            </div>
            <div style={{ borderTop:"1px solid #334155",paddingTop:14,marginBottom:14 }}>
              ...
              <div style={{ display:"flex",gap:8 }}>
                <input value={newClientName} onChange={e => setNewClientName(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && requestClientAction("add", newClientName)}
                  placeholder="例：株式会社〇〇" style={{ ...inp, flex:1, padding:"8px 10px" }}/>
                <button onClick={() => requestClientAction("add", newClientName)}
                  style={{ background:"#1d4ed8",color:"#fff",border:"none",borderRadius:8,padding:"8px 14px",fontWeight:600,fontSize:13,cursor:"pointer",flexShrink:0 }}>追加</button>
              </div>
            </div>
            <button onClick={() => requestClientAction("save", null)}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:50,display:"flex",alignItems:"flex-end",justifyContent:"center" }}
          ...
          <div style={{ background:"#1e293b",borderRadius:"16px 16px 0 0",padding:"20px 18px 32px",width:"100%",maxWidth:520,maxHeight:"92vh",overflowY:"auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background:"#0f172a",borderRadius:10,border:"1px solid #334155",marginBottom:14,overflow:"hidden" }}>
              ...
              <input ref={fileRef} type="file" accept=".pdf,.dwg,.dxf,.xlsx,.xls,.docx,.png,.jpg,.jpeg" multiple
                style={{ display:"none" }} onChange={e => { Array.from(e.target.files).forEach(addUploadFile); e.target.value = ""; }}/>
              {uploadFiles.length === 0 ? (
                <div
                  onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor = "#3b82f6"; }}
                  onDragLeave={e => { e.currentTarget.style.borderColor = "transparent"; }}
                  onDrop={e => { e.preventDefault(); e.currentTarget.style.borderColor = "transparent"; Array.from(e.dataTransfer.files).forEach(addUploadFile); }}
                  style={{ padding:"20px 0",textAlign:"center",color:"#475569",fontSize:12,border:"2px dashed transparent",borderRadius:8,margin:8,cursor:"pointer" }}>
                  ...
                  return (
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderTop:"1px solid #1e293b" }}>
                      ...
                      <button onClick={() => removeUploadFile(i)}
                        style={{ background:"#7f1d1d",border:"none",borderRadius:6,color:"#fca5a5",width:24,height:24,cursor:"pointer",fontSize:12,flexShrink:0 }}>×</button>
                    </div>
                  );
                })
              )}
            ...
            <div style={{ marginBottom:10 }}>
              ...
              <input value={uploadForm.name} onChange={e => setUploadForm({ ...uploadForm, name: e.target.value })}
                placeholder="例：〇〇社 搬送設備" style={inp}/>
            </div>
            <div style={{ marginBottom:10 }}>
              <div style={{ fontSize:11,color:"#64748b",marginBottom:3 }}>見積番号</div>
              <input value={uploadForm.number} onChange={e => setUploadForm({ ...uploadForm, number: e.target.value })}
                placeholder="例：EST-2026-001" style={inp}/>
            </div>
            <div style={{ marginBottom:10 }}>
              ...
              <input value={uploadForm.tags} onChange={e => setUploadForm({ ...uploadForm, tags: e.target.value })}
                placeholder="例：搬送, ベルト" style={inp}/>
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10 }}>
              <div>
                <div style={{ fontSize:11,color:"#64748b",marginBottom:3 }}>客先</div>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:"flex",gap:6,marginBottom:8,flexWrap:"wrap" }}>
                {Object.entries(PROD_STATUS).map(([key, info]) => (
                  <button key={key}
                    onClick={() => setUploadForm({ ...uploadForm, prod_status: key, prod_date: key === "done" ? (uploadForm.prod_date || new Date().toISOString().split("T")[0]) : "" })}
